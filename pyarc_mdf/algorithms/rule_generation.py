...
def generateCARs(transactionDB, assosiation_algorithm="apriori", support=1, confidence=50, maxlen=10, **kwargs):
    ...
    **kwargs : 
    ...
    association_rule_algorithm = None
    if assosiation_algorithm == "apriori": association_rule_algorithm = fim.apriori
    elif assosiation_algorithm == "arules":  association_rule_algorithm = fim.arules
    elif assosiation_algorithm == "eclat": association_rule_algorithm = fim.eclat
    elif assosiation_algorithm == "fpgrowth": association_rule_algorithm = fim.fpgrowth

    rules = None
    if rules == None:
        try : rules = association_rule_algorithm(transactionDB.string_representation, supp=support, conf=confidence, mode="o", target="r", report="sc", appear=appear, **kwargs, zmax=maxlen)
        except : pass

    if rules == None:
        try : rules = association_rule_algorithm(transactionDB.string_representation,  supp=support, conf=confidence, mode="o", report="sc", appear=appear, zmax=maxlen, zmin=2)
        except : pass
...
def top_rules(transactions,
    ...
    while flag:
        ...
        association_rule_algorithm = None
        if assosiation_algorithm == "apriori":
            association_rule_algorithm = fim.apriori
        elif assosiation_algorithm == "arules":
            association_rule_algorithm = fim.arules
        elif assosiation_algorithm == "eclat":
            association_rule_algorithm = fim.eclat
        elif assosiation_algorithm == "fpgrowth":
            association_rule_algorithm = fim.fpgrowth

        rules_current = None
        if rules_current == None:
            try:
                rules_current = association_rule_algorithm(transactions, supp=support, conf=conf,
                                                   mode="o", target="r", report="sc", zmax=maxlen)
            except:
                pass
        ...
        if rules_current == None:
            try:
                rules_current = association_rule_algorithm(transactions, supp=support, conf=conf,
                                                   mode="o", report="sc", zmax=maxlen, zmin=minlen)
            except:
                pass
